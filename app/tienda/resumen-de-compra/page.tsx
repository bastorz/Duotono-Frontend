...
import {
  ...
  ADD_SHIPPING_METHOD,
  ADD_CUSTOMER_FOR_ORDER,
  ADD_SHIPPING_ADDRESS,
  ADD_BILLING_ADDRESS,
  TRANSITION_ORDER_TO_ARRANGING_PAYMENT,
  ADD_PAYMENT_TO_ORDER,
  GET_CLIENT_SECRET,
  TRANSITION_ORDER_TO_CANCELLED,
} from '@/lib/document';
...
export default function Prueba() {
  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      selectedShippingMethod === 'Selecciona una opción'
    ) {
  ...
  return (
    <div className="px-10 md:px-20 xl:px-32 py-20">
      <h3 className="font-bold text-3xl xl:text-4xl pb-10">
        Resumen de compra
      </h3>
      {activeOrder ? (
        <div>
            ...
            {activeOrder.lines.length === 0 ? (
              ...
              <div className="xl:col-span-4 xl:shadow-lg shadow-right shadow-left shadow-bottom xl:p-10">
                {activeOrder?.lines.map((line) => (
                  <div
                    key={line.id}
                    className="grid grid-cols-1 xl:grid-cols-5 py-4 xl:py-10"
                  >
                    <div className="col-span-1">
                      {line.featuredAsset.preview && (
                        <Image
                          ...
                          width={600}
                          height={600}
                          className="h-full w-full object-center rounded-xl pb-8 xl:pb-0"
                        />
                      )}
                    </div>
                    <div className="flex flex-col space-y-4 xl:space-y-2 xl:col-span-4 xl:px-10">
                      <div className="flex flex-col xl:flex-row w-full justify-between">
                        <div className="font-semibold pb-4 text-lg">
            ...
            <div className="flex flex-col space-y-4 xl:w-max">
              <div className="xl:col-span-2 bg-black rounded-xl p-4 xl:p-8">
                ...
                {activeOrder.lines.map((line, i) => (
                  <div
                    className="flex items-center justify-between py-2 xl:py-0 xl:space-x-10"
                    key={i}
                  >
                    <p className=" text-white/80">
                      {line.productVariant.name.length > 30
                        ? line.productVariant.name.substring(0, 30) + '...'
                        : line.productVariant.name}
                    </p>
                    <p className=" text-white/80">
                ...
                <div className="flex flex-col py-6 space-y-10 ">
                  <div className="flex flex-col space-y-2 ">
                    ...
                    <div className="flex justify-between">
                      ...
                      <p className=" text-white/80">
                        {formatCurrency(activeOrder.totalWithTax)}
                      </p>
                    </div>
                  </div>
                  <Link href="#datosdeenvio">
                    <Button
                      variant="default"
                      className={cn(
                        'bg-main gap-x-4 w-full text-black bg-first'
                      )}
                    >
                      <ShoppingCart />
          ...
          <div className="py-10" id="datosdeenvio">
            <h5 className="font-bold text-3xl xl:text-4xl pb-10">
              Configura tu envío
            </h5>
            <div
              className={cn(
                'px-4 py-8 xl:p-8 border border-black/40 rounded-md',
                successConfiguration ? 'border-green-500' : ''
              )}
            >
              <p className="font-semibold">Datos de envío</p>
              <div className="h-[1px] bg-black my-6 " />
              <form
                className="grid grid-cols-2 xl:grid-cols-4 gap-x-4 gap-y-10 xl:gap-x-10 xl:gap-y-6"
              ...
              >
                ...
                <div className="flex flex-col space-y-2 col-span-2 xl:col-span-1">
                  ...
                  >
                    ...
                    {eligibleShippingMethodsData?.eligibleShippingMethods.map(
                      (method) => (
                ...
                <Button
                  type="submit"
                  className={cn(
                    'bg-main gap-x-4 text-black bg-first mt-8 font-semibold col-span-2 xl:col-span-1'
                  )}
                >
                  {isLoading ? (
                    <div className="h-screen flex items-center justify-center">
          ...
          <div className="py-0 xl:py-10">
            ...
            <div
              className={cn(
                'px-8 py-10 m-4 cursor-pointer border border-black/40 rounded-md flex flex-col xl:flex-row items-center justify-between space-y-4 xl:space-y-0 xl:space-x-10',
                selectedStripe ? 'bg-black-500/5 border-black' : 'bg-white'
              )}
                ...
                <Dialog>
                  <DialogTrigger>
                    <Button
                      variant="default"
                      className={cn(
                        'bg-main gap-x-4 w-64 xl:w-80 text-black bg-first'
                      )}
                    >
                      <ShoppingCart />
                      <p className="text-[12px] xl:text-[16px]">
                        Pagar a través de Stripe
                      </p>
                      <ArrowRight />
                    </Button>
                  </DialogTrigger>
