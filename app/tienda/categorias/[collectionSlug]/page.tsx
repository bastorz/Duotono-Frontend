...
const Page = ({params}: {params: {collectionSlug: string}}) => {
  ...
  const filteredProducts = collectionsData?.collection.productVariants.items.filter((item) => {
              ...
              {collectionsData?.collection.children.map((subcategory) => (
                  <div className="flex flex-col h-full justify-between py-10">
                    ...
                    <div className="flex flex-col space-y-4 pt-4">
                      ...
                      <p className="text-black/70 xl:max-w-3xl xl:text-xl max-w-[20rem]">{removeHTMLTags(subcategory.description)}</p>
                      <Link href={`/tienda/categorias/${subcategory.slug}`} className="flex space-x-1 items-center  group">
                ...
                {filteredProducts?.map((item) => (
                  <div key={item.name} className="flex flex-col space-y-4 ">
                    <Link href={`/tienda/productos/${item.product.slug}`} className="min-h-[300px] relative group">
                            <Image src={item.product.featuredAsset.preview} alt="" width={600} height={600} className="object-center max-w-[300px] transition-transform transform-gpu duration-300 group-hover:scale-110"/>
                            <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 ">
                            </span>
                    </Link>
                    <h3 className="font-bold text-2xl">{item.product.name}</h3>
                    <p className="text-gray-500 max-w-[300px]">{removeHTMLTags(item.product.description)}</p>
                    <p className="text-gray-500">1000 unidades por</p>
                    <div className="flex space-x-5">
                      <p className="text-red-500">{formatCurrency(item.product.customFields.PrecioBase)}</p>
                      <p className="font-bold text-xl">{formatCurrency(item.product.customFields.Precio1000Unidades)}</p>
                    </div>
                  </div>
                ))}
          </div>
